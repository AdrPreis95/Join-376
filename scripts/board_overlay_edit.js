...
//   initAssignedChangeTracking(); snapshotAssignedInit();
// };
...
// /** ===== Assigned-Änderungen: Snapshot + Observer ===== */
// let _assignedInit = [];
// let _assignedMO;

// /** Liest aktuelle Auswahl (Chips bevorzugt, sonst Checkboxen). */
...
//     if(muts.some(m=>m.type==='childList'&&(m.addedNodes.length||m.removedNodes.length))) _sessionChanged=true;
//   });
//   _assignedMO.observe(root,{childList:true,subtree:true});
// };

// /** Optional: Observer stoppen (z. B. im Overlay-Close). */
...
//       dd.classList.add('d-none'); dd.classList.remove('d_block'); arr.src='./assets/icons/arrow_drop_down.png'; box.classList.remove('d-none');
//     }
//   });
// });
...
// /* ===== Upload-/User-Aktionen als Änderung markieren (robust) ===== */
...
// document.addEventListener('click', (e) => {
//   if (e.target.closest('[data-file-delete], .file-delete, .remove-file')) { _sessionChanged = true; return; }
...
function warnInlineSubtaskEdit(){ showEditConfirm('Please change the subtask and confirm with Enter','warn'); }
