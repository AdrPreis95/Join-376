...
//    /** Picks a random element from the POOL array. */
//   function pick() {
...
//   /** Derives a deterministic element from POOL based on a given key. */
//   function derive(k) {
//     return POOL[hash(k) % POOL.length];
//   }
...
(function () {
  ...
  function colorFromStorageOrDerive(key){
    const lsKey = "contactColor:" + key;
    let col = localStorage.getItem(lsKey);
    if (!col) { col = derive(lsKey); localStorage.setItem(lsKey, col); }
    return col;
  ...
  function stableColor(arg) {
    ...
    if (arg == null) return pick();
    if (typeof arg === "object") {
      const col = colorFromStorageOrDerive(objKey(arg));
      arg.color = arg.color || col; return arg.color;
    }
    const k = "contactColor:" + String(arg).trim().toLowerCase();
    let col = localStorage.getItem(k);
    if (!col) { col = derive(k); localStorage.setItem(k, col); }
